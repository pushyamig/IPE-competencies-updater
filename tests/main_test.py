...
def worksheet_columns():
    ...
    """
    return ['Offering Name','Offering ID','Offering-instance ID','Year','Term ','UM Term Code','udp_term_name','Description','Lead Faculty Member 1','Lead Faculty Member 2','Lead Faculty 1 School','Lead Faculty 2 School','Lead Faculty 1 unique name','Lead Faculty 2 unique name','Format','Instructional Team\n(SEE LOOKUP TABLE)','Program offering to students\n(SEE LOOKUP TABLE)','Course #s (if applicable)','Window used','IPE Assessments','Canvas Course ID','Roles & Responsibilities','Interprofessional Communication','Teams/Teamwork','Values/Ethics','Intercultural Humility ','IPE competency dosage/ Contact Hours','Criteria for Assigning Outcomes in Canvas','Notes','When does script run? (Feb, June)','Script Run?']
...
def test_non_course_id_clean_up(ipe_ws_df):
    ...
    logging.info(list(df_after['Canvas Course ID']))
...
def test_clean_up_df(ipe_ws_df, ipe_props):
    ...
    """
    orc = IPECompetenciesOrchestrator(ipe_props, ipe_ws_df, api_handler)
